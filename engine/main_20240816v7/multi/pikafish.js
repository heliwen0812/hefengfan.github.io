...
if (!D) {
  if (B.wasmMemory) {
    ...
    if (m = new WebAssembly.Memory({initial:(B.INITIAL_MEMORY || 67108864) / 65536, maximum:16384, shared:!0}), !(m.buffer instanceof SharedArrayBuffer)) {
...
var Cb = () => navigator.hardwareConcurrency, Db = a => {
  ...
  for (var c = 1; 4 >= c; c *= 2) {
    ...
    d = Math.max(a, d);
    a: {
      e = (e.min.call(e, 1073741824, d + (65536 - d % 65536) % 65536) - m.buffer.byteLength + 65535) / 65536;
      try {
        m.grow(e);
        x();
        var f = 1;
